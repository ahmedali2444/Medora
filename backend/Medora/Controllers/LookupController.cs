...
public class LookupController : ControllerBase
{
	...
	{
		return Ok(await (from s in _db.Specialties
			where !s.IsArchived
			select new
			{
				id = s.Id,
	...
	{
		return Ok(await (from g in _db.Governorates
			where !g.IsArchived
			select new
			{
				id = g.Id,
	...
	{
		return Ok(await (from c in _db.Cities
			where c.GovernorateId == governorateId && !c.IsArchived && !c.Governorate.IsArchived
			select new
			{
				id = c.Id,
				nameAr = c.NameAr,
