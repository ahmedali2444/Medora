...
function buildPrimaryNav(isRtl, badges = {}) {
  return [
    { id: 'overview', icon: LayoutDashboard, label: isRtl ? 'نظرة عامة' : 'Overview', path: '/doctor/overview' },
    { id: 'appointments', icon: CalendarCheck, label: isRtl ? 'المواعيد' : 'Appointments', path: '/doctor/appointments', badge: badges.appointments },
    { id: 'patients', icon: Users, label: isRtl ? 'المرضى' : 'Patients', path: '/doctor/patients' },
    { id: 'prescriptions', icon: ClipboardList, label: isRtl ? 'الروشتات' : 'Prescriptions', path: '/doctor/prescriptions' },
    { id: 'reviews', icon: MessagesSquare, label: isRtl ? 'التقييمات' : 'Reviews', path: '/doctor/reviews' },
    { id: 'clinics', icon: Stethoscope, label: isRtl ? 'العيادات' : 'Clinics', path: '/doctor/clinics' },
  ];
}

function buildSecondaryNav(isRtl) {
  return [
    { id: 'home', icon: Home, label: isRtl ? 'الصفحة الرئيسية' : 'Main website', path: '/' },
    { id: 'profile', icon: FileText, label: isRtl ? 'الملف الشخصي' : 'Profile', path: '/doctor/profile' },
    { id: 'settings', icon: Settings, label: isRtl ? 'الإعدادات' : 'Settings', path: '/doctor/settings' },
  ];
}

export default function DoctorSidebar({ open, onNavigate, onToggleSidebar, expanded, badges = {} }) {
  const navigate = useNavigate();
  const { lang } = useLang();
  const { logout } = useAuth();
  const isRtl = lang !== 'en';
  const primaryNav = buildPrimaryNav(isRtl, badges);
  const secondaryNav = buildSecondaryNav(isRtl);
      ...
      >
        {({ isActive }) => (
          <>
            ...
            {expanded && hasBadge && (
              <span
                className={`rounded-full px-1.5 text-[10px] font-black ${
                  isActive ? 'bg-white/20 text-white' : 'bg-[#14b8a6] text-white'
                }`}
              >
                {item.badge}
              </span>
            ...
            {!expanded && hasBadge && (
              <span className="absolute -top-0.5 -right-0.5 min-w-4 rounded-full bg-[#14b8a6] px-1 text-center text-[9px] font-black leading-4 text-white">
  ...
  return (
    <>
      ...
      >
        <div className={`flex items-center border-b border-[#e4eeee] py-4 ${expanded ? 'justify-between gap-3 px-3' : 'justify-center px-2'}`}>
          <button
            type="button"
            ...
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="truncate text-[11px] text-slate-500">
                  {isRtl ? 'منصة الأطباء' : 'Doctors platform'}
                </div>
              </div>
            </div>
          )}
        ...
        <div className={`flex-1 overflow-y-auto ${expanded ? 'px-3 py-4' : 'px-2 py-3'}`}>
          ...
          <nav className={`flex flex-col ${expanded ? 'gap-1' : 'gap-2'}`}>{primaryNav.map(renderLink)}</nav>
          ...
          <nav className={`flex flex-col ${expanded ? 'gap-1' : 'gap-2'}`}>{secondaryNav.map(renderLink)}</nav>
        </div>
