...
export default function AddReviewModal({ open, onClose, doctorId, appointmentId, onSubmitted }) {
  const { lang } = useLang();
  const isRtl = lang !== 'en';
  ...
  const submit = async (e) => {
    ...
    try {
      await medoraApi.createReview({ targetType: 'doctor', doctorId, appointmentId, rating, comment });
      onSubmitted?.();
      onClose?.();
    } catch (err) {
      setError(err?.message || (isRtl ? 'تعذر إرسال التقييم' : 'Failed to submit review'));
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/40 p-4" dir={isRtl ? 'rtl' : 'ltr'}>
      <form onSubmit={submit} className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
        <div className="mb-5 flex items-center justify-between"><h3 className="text-lg font-black text-[#084036]">{isRtl ? 'تقييم الطبيب' : 'Review doctor'}</h3><button type="button" onClick={onClose}><X size={20} /></button></div>
