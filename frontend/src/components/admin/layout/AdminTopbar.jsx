...
import { Bell, ChevronDown, Home, LogOut, ScanSearch as Search, ShieldCheck } from 'lucide-react';
...
export default function AdminTopbar({
  ...
  const goTo = (path) => {
    const shouldNavigate = onRequestNavigate ? onRequestNavigate(path) : true;
    if (shouldNavigate) {
      navigate(path);
    }
  };
    ...
    >
      ...
      <div className="hidden max-w-md flex-1 md:block">
        <div className="relative">
          <input
            type="search"
            placeholder={isRtl ? 'ابحث في كل المنصة...' : 'Search across the whole platform...'}
            className={[
            ...
            <div className={`fixed inset-x-3 top-[70px] z-50 overflow-hidden rounded-2xl border border-[#e4eeee] bg-white shadow-[0_24px_50px_rgba(8,64,54,0.18)] sm:absolute sm:inset-x-auto sm:top-auto sm:mt-2 sm:w-80 ${dropdownSideClass}`}>
              <div className="flex items-center justify-between border-b border-[#e4eeee] px-4 py-3">
                ...
                <span className="text-[12px] font-extrabold text-[#084036]">
                  {isRtl ? 'إشعارات المنصة' : 'Platform notifications'}
                </span>
              </div>
        ...
        <div className="relative">
          ...
          >
            ...
            <div className={`hidden sm:block ${isRtl ? 'text-start' : 'text-left'}`}>
              <div className="text-[12px] font-bold text-[#084036]">{isRtl ? 'أحمد المسؤول' : 'Ahmed Admin'}</div>
              <div className="text-[10px] text-slate-500">Super Admin</div>
            </div>
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-[#0b5e52] to-[#14b8a6] text-white sm:h-9 sm:w-9">
              <ShieldCheck size={15} />
            </div>
          </button>
